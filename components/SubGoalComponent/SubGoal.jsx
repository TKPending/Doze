...
const SubGoal = () => {
  ...
  return (
    <div className="bg-gray-100 md:h-3/4 md:w-1/2 w-4/5 h-4/5 p-6 rounded-lg mt-24 outline outline-[#FF9796]">
        <form className="flex flex-col items-center w-full h-full" onSubmit={formHandler}>
        

        <button onClick={toggleEmojiPicker} className="mb-5"><span className="text-6xl">{emojiPicture()}</span></button>
        {isOpen && <Picker data={data} onEmojiSelect={handleEmoji} onClickOutside={toggleEmojiPicker} maxFrequentRows={0} />}

        <input type="text" className="input input-bordered w-1/2 mb-5 border-[#FF9796] focus:border-[#FF9796] focus:outline-[#FF9796]" defaultValue="Title"></input>
        ...
        <input name="tag" type="text" value={tagInput} onChange={handleTagInputChange} className="input focus:border-[#FF9796] focus:outline-[#FF9796] w-64 border-[#FF9796]" placeholder="Tags"></input>

        <button className="bg-[#FF9796] text-white rounded-md p-2.5 ml-4" onClick={handleAddTag}>+</button>
        </div>
            ...
            {tags.map((tag, index) => (
                <span key={index}>
                    <div className={`badge bg-${tag.colour}-400 gap-2 p-4`}>
                        {tag.text}
                        <a onClick={() => handleRemoveTag(index)} className="inline-block cursor-pointer w-2.5">x</a>
                    </div>
                </span>
            ))}
        </div>

        <textarea className="textarea input-bordered w-1/2 mb-5 border-[#FF9796] focus:border-[#FF9796] focus:outline-[#FF9796]" placeholder="Additional information"></textarea>

        

              <button className="absolute right-96 bottom-16 text-[#ff9796] hover:text-white hover:bg-[#ff9796] border border-[#ff9796] rounded-md p-2 pl-5 pr-5">Save</button>
        </form>
